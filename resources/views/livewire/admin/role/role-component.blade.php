                            ...
                            <tbody>
                                @foreach ($roles as $key => $role)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="#"
                                                class="text-body">{{ ucfirst(Str_replace('-', ' ', $role->name)) }}</a>
                                        </td>
                                        <td>
                                            @foreach ($role->permissions as $key => $permissions)
                                                @if (($key + 1) % 4 == 0)
                                                    <span
                                                        class="badge rounded-pill bg-secondary">{{ ucwords(str_replace('-', ' ', $permissions->name)) }}</span></br>
                                                @else
                                                    <span
                                                        class="badge rounded-pill bg-secondary">{{ ucwords(str_replace('-', ' ', $permissions->name)) }}</span>
                                                @endif
                                            @endforeach
                                        </td>
                                        @canany(['role-edit', 'role-delete'])
                                                    ...
                                                    @can('role-edit')
                                                        <li class="list-inline-item">
                                                            <a href="javascript:void(0);" class="px-2 text-primary"
                                                                wire:click.prevent='editRole({{ $role }})'><i
                                                    ...
                                                    @can('role-delete')
                                                        <li class="list-inline-item">
                                                            <a href="javascript:void(0);" class="px-2 text-danger"
                                                                wire:click.prevent='deleteConfirmation({{ $role->id }})'><i
                        ...
                        <div class="row g-2">
                            <div class="col mb-0">
                                ...
                                <input type="text" id="role-name"
                                    ...
                                    placeholder="Enter Role Name" wire:model.lazy='name' />
                                @error('name')
                                        ...
                                        @foreach ($permissionGroup as $key1 => $per)
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox"
                                                    id="formCheck{{ $per->id }}" value="{{ $per->id }}"
                                                    wire.key="{{ $per->id }}" wire:model.lazy='permissionsId'>
