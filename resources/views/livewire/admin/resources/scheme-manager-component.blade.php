                            ...
                            <tbody>
                                @foreach ($schemes as $key => $scheme)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @can('scheme-manager-status-change')
                                                <input type="checkbox" id="switch{{ $scheme->id }}" switch="bool"
                                                    @if ($scheme->status == 1) checked @endif
                                                    wire:change='statusUpdate({{ $scheme->id }},{{ $scheme->status }})' />
                                                <label for="switch{{ $scheme->id }}" data-on-label="Active"
                                        ...
                                        <td>
                                            <ul class="list-inline mb-0">
                                                <li class="list-inline-item"
                                                    wire:click.prevent="edit({{ $scheme }})">
                                                ...
                                                <li class="list-inline-item dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        ...
                                                        <a class="dropdown-item" href="javascript:void()"
                                                            wire:click.prevent="getCommission({{ $scheme->id }},'mobile')">Mobile
                                                            Recharge</a>
                                                        <a class="dropdown-item" href="javascript:void()">Virtual
                                                            Account</a>
                                                        <div class="menu-header">Charge</div>
                                                        <a class="dropdown-item" href="javascript:void()"
                                                            wire:click.prevent="getCommission({{ $scheme->id }},'dmt')">Money
                                                            Transfer</a>
                                                        <a class="dropdown-item" href="javascript:void()">Virtual
                                                            Account</a>
                                                    </div>
                                    ...
                                    <ul class="pagination mb-sm-0">
                                        ...
                                        @foreach (range(1, $schemes->lastPage()) as $i)
                                            @if ($i >= $schemes->currentPage() - 2 && $i <= $schemes->currentPage())
                                                <li class="page-item @if ($schemes->currentPage() == $i) active @endif"
                                        ...
                                        @if ($schemes->currentPage() < $schemes->lastPage() - 2)
                                            <li class="page-item" wire:click="gotoPage({{ $schemes->lastPage() }})">
                                                <a href="javascript:void(0)"
                                                    class="page-link">{{ $schemes->lastPage() }}</a>
                                            </li>
                                        @endif
                ...
                <form wire:submit.prevent='{{ $editSchemeForm ? 'update' : 'store' }}' autocomplete="off">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="myLargeModalLabel">{{ $editSchemeForm ? 'Edit' : 'Add' }} Scheme
                            </h5>
                            ...
                            <div class="row g-2">
                                <div class="col mb-0">
                                    ...
                                    <input type="text" id="scheme-name"
                                        class="form-control  @error('schemeName') is-invalid @enderror"
                                        placeholder="Enter Scheme Name" wire:model.defer='schemeName' />
                                    @error('schemeName')
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-primary">{{ $editSchemeForm ? 'Update' : 'Save' }}
                                changes</button>
                        </div>
                    </div><!-- /.modal-content -->
                </form>
                                    ...
                                    <tbody>
                                        @foreach ($operaterList as $key => $item)
                                            <tr>
                                                ...
                                                <input type="hidden" type="hidden" wire:model="items"
                                                    value="{{ $item->id }}">
                                                <td>
                                                    <select wire:model.defer="slab.{{ $key }}.type"
                                                        class="form-control">
